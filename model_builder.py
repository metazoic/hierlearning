...
for a in range(num_actions):
	...
	for x in range(num_variables):
                if not prune:
        		model_str = subprocess.Popen(["java", "weka.classifiers.trees.J48", "-v", "-no-cv", "-U", "-M", "1", "-t", directory + actions[a] + "__" + state_variables[x] + ".arff"], stdout = subprocess.PIPE).communicate()[0]
       		else:
        		model_str = subprocess.Popen(["java", "weka.classifiers.trees.J48", "-v", "-no-cv", "-C", str(confidence), "-M", "1", "-t", directory + actions[a] + "__" + state_variables[x] + ".arff"], stdout = subprocess.PIPE).communicate()[0]
	...
	if len(reward_nominal_values[a]) > 1:
                if not prune:
        		model_str = subprocess.Popen(["java", "weka.classifiers.trees.J48", "-v", "-no-cv", "-U", "-M", "1", "-t", directory + actions[a] + "__reward.arff"], stdout = subprocess.PIPE).communicate()[0]
        	else:
        		model_str = subprocess.Popen(["java", "weka.classifiers.trees.J48", "-v", "-no-cv", "-C", str(confidence), "-M", "1", "-t", directory + actions[a] + "__reward.arff"], stdout = subprocess.PIPE).communicate()[0]
